...
public class Spiral {
	...
	enum Qaudrant {
		...
		public static Qaudrant calculate(int x, int z) {
			if (z > 0) {
				if (x > 0)
					return TOP_RIGHT;
				else
	...
	public Spiral(MMCoord center, int radius) {
		this.radius = radius;
	...
	private boolean turn() {
		...
		//P.p.logQueue.add(new Pair<Level,String>(Level.INFO, qaud.toString()));
		switch (qaud) {
		case BOT_RIGHT:
			// When we move into the next layer of the square we check if the limit is valid
			// We also need to check if we are within the circle because after we leave for the first time we must check it on every pass
			//P.p.logQueue.add(new Pair<Level,String>(Level.INFO, String.format("Limit is now %d", limit + 1)));
